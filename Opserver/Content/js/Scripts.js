    ...
    function init(options) {
        ...
        Status.loaders.register({
            ...
            '#/db/': function (val, firstLoad, prev) {
                var obj = val.indexOf('tables/') > 0 || val.indexOf('views/') || val.indexOf('storedprocedures/') || val.indexOf('unusedindexes/') > 0 || val.indexOf('querystore/') > 0 || val.indexOf('blitzcache/') > 0
                          ? val.split('/').pop() : null;
                ...
                if (!firstLoad) {
                    ...
                    if ((/\/tables/.test(val) && /\/tables/.test(prev)) || (/\/views/.test(val) && /\/views/.test(prev)) || (/\/storedprocedures/.test(val) && /\/storedprocedures/.test(prev)) || (/\/unusedindexes/.test(val) && /\/unusedindexes/.test(prev)) || (/\/querystore/.test(val) && /\/querystore/.test(prev)) || (/\/blitzcache/.test(val) && /\/blitzcache/.test(prev))) {
                        showColumns();
                        return;
                    }
                }
        ...
        $(document).on('click', function () {
            ...
            window.location.hash = window.location.hash.replace(/\/tables\/.*/, '/tables').replace(/\/views\/.*/, '/views').replace(/\/storedprocedures\/.*/, '/storedprocedures').replace(/\/querystore\/.*/, '/querystore').replace(/\/blitzcache\/.*/, '/blitzcache');
        });
    }
